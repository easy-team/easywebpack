...
class Config {
  ...
  createMediaName() {
    const prefix = this.config.prefix;
    if (this.config.mediaHash) {
      this.config.mediaName = utils.assetsPath(prefix, `media/[name].[hash:${this.config.hashLength}].[ext]`);
    } else {
      this.config.mediaName = utils.assetsPath(prefix, 'media/[name].[ext]');
    }
  }
  ...
  setDevTool(devtool, force) {
    ...
    const cliDevtool = this._config.cliDevtool;
  ...
  setMediaHash(isHash) {
    this.setConfigValueByKey('mediaHash', isHash);
  }
  ...
  setBuildPath(buildPath, isOverride = false) {
    buildPath = !this.config.egg && !isOverride && this.outerConfig.buildPath ? this.outerConfig.buildPath : buildPath;
