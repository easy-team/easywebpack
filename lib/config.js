...
class Config {
  constructor(config) {
    ...
    this.webpackNodeList = [
      'context', 'target', 'node', 'devtool', 'output', 'externals', 'resolve', 'watch', 'watchOptions', 'amd',
      'resolveLoader', 'devServer', 'performance', 'module', 'cache', 'profile'
    ];
  ...
  initConfig(config) {
    ...
    this.outerWebpackConfig = this.analysisWebpackConfig(this.outerConfig);
  ...
  setPublicPath(publicPath, isOverride = true) {
    ...
    if (this.utils.has(publicPath)) {
      ...
      if (this.dev) {
        this.config.publicPath = this.setHostPublicPath(this.config.publicPath);
      }
    }
  }
  ...
  setCDN(url, dynamicDir) {
    if (this.utils.has(url)) {
      ...
      if (cdnUrl) {
        const publicPath = cdnDir ? this.utils.joinPath(cdnUrl, cdnDir) : cdnUrl;
  ...
  setHostPublicPath(publicPath) {
    ...
    if (publicPath.includes(this.host)) {
      return publicPath;
    }
  ...
  setDevMode() {
    this.setHostPublicPath(this.config.publicPath);
