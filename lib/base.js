...
class WebpackBaseBuilder {
  ...
  create() {
    if(this.config.createBefore){
      this.config.createBefore.bind(this)();
    }
    this.createInit();
    if(this.config.createAfter){
      this.config.createAfter.bind(this)();
    }
  ...
  initConfig() {
    ...
    this.setCDN(config.cdnUrl, config.cdnDynamicDir);
