      ...
      if (Array.isArray(itemLoader.use)) {
        ...
        itemLoader.use.forEach((loader, index) => {
          if (utils.isString(loader)) {
            const options = this.merge(loaderOptions[loader] || loaderOptions[loader.replace('-loader', '')], itemLoader.options);
            options && itemLoader.use.splice(index, 1, { loader, options });
          } else if (utils.isObject(loader) && utils.isString(loader.loader)) {
            let options = loaderOptions[loader.loader] || loaderOptions[loader.loader.replace('-loader', '')];
            if (itemLoader.use.length === 1 || name === loader.loader.replace('-loader', '')) {
              options = this.merge(options, itemLoader.options);
            }
