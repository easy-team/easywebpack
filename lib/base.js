...
class WebpackBaseBuilder {
  ...
  getDllFilePath() {
    const filename = this.config.plugins && this.config.plugins.dll && this.config.plugins.dll.filename || 'dll.json';
    return path.isAbsolute(this.buildPath) ? path.join(this.buildPath, filename) :
      path.join(this.config.baseDir, this.buildPath, filename);
  }
}
