...
class WebpackBaseBuilder {
  ...
  initOption() {
    this.setOption({ resolveLoader: { modules: [path.join(__dirname, '../node_modules'), 'node_modules'] } });
  }
  ...
  addBuildConfig() {
    if (this.config.buildConfig) {
      ...
      const filepath = path.posix.join(this.config.baseDir, 'config/buildConfig.json');
      const buildConfig = {
  ...
  addManifest() {
    this.addPlugin(ManifestPlugin, () => {
      ...
      const manifestPath = path.posix.isAbsolute(manifest) ? manifest : path.posix.join(this.config.baseDir, manifest);
      const fileName = path.posix.relative(this.buildPath, manifestPath);
      return { fileName };
