...
class WebpackBaseBuilder {
  constructor(config = {}) {
    ...
    this.loaders = cloneDeep(require('../config/loader'));
    this.plugins = cloneDeep(require('../config/plugin'));
  ...
  mergeLoader(loaders, target) {
    ...
    const sourceLoaders = cloneDeep(loaders);
  ...
  mergePlugin(plugins, target) {
    ...
    const sourcePlugins = cloneDeep(plugins);
    Object.keys(sourcePlugins).forEach(name => {
      ...
      if (target[name]) {
        if (utils.isBoolean(configPlugin)) {
          ...
          if (configPlugin.hasOwnProperty('enable')) { // config enable > plugin.enable
            target[name].enable = configPlugin.enable;
          }
  ...
  isUse(name, range = 'plugin') {
    const configInfo = utils.isObject(name) ? name : (range === 'plugin' ? this.getPluginByName(name) : this.getLoaderByName(name));
    return this.isEnable(configInfo.enable) && this.isType(configInfo.type) && this.isEnv(configInfo.env);
  }
  ...
  prepareLoader(loaders, loaderOptions) {
    const extract = this.isUse('extract');
      ...
      if (Array.isArray(itemLoader.use)) {
        ...
        itemLoader.use.forEach((loader, index) => {
          if (utils.isString(loader)) {
            ...
            const options = loaderOptions[loader.loader] || loaderOptions[loader.loader.replace('_loader', '')];
            if (options) {
  ...
  createLoader(loaders) {
    ...
    Object.keys(loaders).forEach(name => {
      const itemLoader = loaders[name];
      if (this.isEnable(itemLoader.enable)) {
  ...
  createWebpackLoader() {
    ...
    this.prepareLoader(this.loaders, this.config.loaderOptions);
