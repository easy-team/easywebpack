...
class WebpackBaseBuilder {
  ...
  init() {
    this.config = this.initEnv(this._config);
  ...
  initConfig() {
    this.options = this.utils.cloneDeep(require('../config/option'));
    this.loaders = this.utils.cloneDeep(require('../config/loader'));
    this.plugins = adapter.normalizePlugin(this.config);
  ...
  initEnv(config = {}) {
    this.env = config.env || (this.config && this.config.env) || 'dev'; // local/dev, test, prod
    this.dev = false;
    this.test = false;
    this.prod = false;
    if (this.env === 'prod') {
      this.prod = true;
      return merge(Config.defaultConfig, Config.prodConfig, config);
    } else if (this.env === 'test') {
      this.test = true;
      return merge(Config.defaultConfig, Config.testConfig, config);
    } else {
      this.dev = true;
      return merge(Config.defaultConfig, Config.devConfig, config);
    }
  }
  ...
  setDefault(config) {
    this.config = this.initEnv(config);
    this.setConfig(this._config);
  }

  setConfig(config) {
    this.config = merge(this.config, config);
    this.initialize(this.config, true);
  }

  setEnv(env, config) {
    this.setDefault(merge({ env }, config));
  }
  ...
  mergePlugin(plugins = {}, target) {
    ...
    Object.keys(sourcePlugins).forEach(name => {
      ...
      if (target[name]) {
        ...
        if (!utils.isFunction(target[name].enable)) {
          // 如果配置了,表示开启插件
          if (utils.isObject(configPlugin) && configPlugin.enable === undefined) {
        ...
        else if (configPlugin.name || this.isWebpackPluin(configPlugin)) { // 直接覆盖
      ...
      } else if (this.isWebpackPluin(configPlugin) || this.isConfigPluin(configPlugin)) {
        target[name] = configPlugin;
      }
    });
  ...
  isWebpackPluin(plugin) {
    return utils.isObject(plugin) && plugin.constructor && plugin.constructor.prototype && plugin.constructor.prototype.apply;
  }

  isConfigPluin(plugin) {
    return utils.isObject(plugin) && plugin.name;
  }
  ...
  createOption() {
    ...
    this.setOption(this.config.options);
  }
  ...
  createPlugin(plugins) {
    ...
    Object.keys(plugins).forEach(name => {
      ...
      if (this.isUse(configInfo)) {
        ...
        let pluginName;
        // 如果直接是 new Plugin 方式
        if (this.isWebpackPluin(configInfo)) {
          plugin = configInfo;
          pluginName = configInfo.constructor.name;
        }
        else if (utils.isObject(configInfo.name)) { // plugin object
          ...
          } else if (utils.isFunction(configInfo.name)) {
  ...
  setLibrary(library){
    this.options.output.library = library;
  }

  setLibraryTarget(libraryTarget){
    this.options.output.libraryTarget = libraryTarget;
  }
  ...
  setMiniCss(miniCss) {
    this.loaders.options = merge(this.loaders.options, {
      css: {
        options: {
  ...
  addEntry(name, value) {
    ...
    Object.keys(entry).forEach(name => {
      this.options.entry[name] = entry[name];
    });
  }
  ...
  addPack(name = {}, value, useCommonsChunk = false) {
    if (utils.isObject(name)) {
      ...
      const normalizeFiles = files.map(file => {
        return utils.normalizePath(file, this.baseDir);
      });
  ...
  setBuildPath(buildPath) {
    this.buildPath = utils.normalizePath(buildPath, this.baseDir);
  ...
  setDevMode(port, proxy) {
    ...
    this.host = utils.getHost(this.config.port);
    if (proxy !== undefined) {
      this.setProxy(proxy)
    }
    // 如果不是当前宿主运行, 设置编译的ip:port
    if (!this.config.currentHost) {
      this.setPublicPath(utils.joinPath(this.host, this.publicPath.replace(this.host, '')));
    }
      ...
      dllArray.forEach(item => {
        if (item.name && item.lib) {
          ...
          plugin[`${item.name}Dll`] = new this.webpack.DllPlugin({
            ...
            path: utils.getDllFilePath(item.name),
            name: '[name]'
          });
  ...
  createDllReferencePlugin(dll) {
    ...
    if (dll) {
      ...
      dllArray.forEach(item => {
        if (item.name && item.lib) {
          const plugin = {};
          const filename = utils.getDllFilePath(item.name);
