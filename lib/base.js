...
class WebpackBaseBuilder {
  ...
  initOption() {
    ...
    const normalizeEntryDirs = entryDirs.forEach(entryDir => {
      return Utils.normalizePath(entryDir, this.config.baseDir);
    });
    this.options = {
  ...
  setAlias(name, value) {
    const alias = {};
    alias[name] = Utils.normalizePath(value, this.config.baseDir);
    this.options = merge(this.options, {
      resolve: { alias }
    });
  }
