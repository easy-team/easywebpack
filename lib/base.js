...
class WebpackBaseBuilder {
      ...
      if (Array.isArray(itemLoader.use)) {
        ...
        itemLoader.use.forEach((loader, index) => {
          if (utils.isString(loader)) {
            const options = itemLoader.use.length === 1 || loader.replace('-loader', '') === name ? itemLoader.options : {};
            const loaderExtend = loaderOptions[name] || loaderOptions[loader.replace('-loader', '')];
            console.log('111',name, loaderExtend);
            itemLoader.use.splice(index, 1, this.merge({ loader }, loaderExtend, { options }));
          } else if (utils.isObject(loader) && utils.isString(loader.loader)) {
            const options = itemLoader.use.length === 1 || loader.loader.replace('-loader', '') === name ? itemLoader.options : {};
            const loaderExtend = loaderOptions[name] || loaderOptions[loader.loader.replace('-loader', '')];
            console.log('222',name, loaderExtend);
            itemLoader.use[index] = this.merge(itemLoader.use[index], loaderExtend, { options });
          }
        });
      }
  ...
  createLoader(loaders) {
    ...
    // except loaders.options, this `options` property is global loader options key
  ...
  installLoader(loaders) {
    ...
    this.utils.installLoader(webpackLoaders, dependencies, modules);
    return webpackLoaders;
  }
  ...
  installPlugin(plugins) {
    ...
    this.utils.installPlugin(enablePlugins, dependencies, modules);
  }
  ...
  initEnv(config = {}, reset) {
    ...
    if (this.env === 'test') {
      ...
      this.prod = false;
      if (reset) {
        this.config = merge(this.config, Config.defaultConfig, Config.testConfig, config);
      } else {
      ...
      this.prod = true;
      if (reset) {
        this.config = merge(this.config, Config.defaultConfig, config);
      } else {
      ...
      this.prod = false;
      if (reset) {
        this.config = merge(this.config, Config.defaultConfig, Config.devConfig, config);
      } else {
