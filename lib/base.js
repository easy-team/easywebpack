...
class WebpackBaseBuilder extends Config {
    ...
    dll.forEach(item => {
      ...
      if (fs.existsSync(manifestFile)) {
        ...
        Object.keys(manifest).forEach(id => {
          const publicPath = this.publicPath.replace(this.host, '');
          const outputName = manifest[id].replace(publicPath, '');
          const outputPath = this.utils.joinPath(this.publicPath, outputName);
  ...
  getManifestFilePath() {
    const plugins = this.outerConfig.plugins || {};
    const manifest = plugins.manifest || {};
    const defaultFileName = this.config.egg ? 'config/manifest.json' : 'manifest.json';
    const configFileName = manifest.fileName || (manifest.args && manifest.args.fileName);
    const baseDir = this.config.egg ? this.baseDir : this.buildPath;
    return path.resolve(baseDir, configFileName || defaultFileName);
  }
  ...
  createCacheLoader(cacheOptions) {
    const cacheDirectory = this.utils.getCacheLoaderInfoPath(this.env);
    const options = this.utils.isObject(cacheOptions) ? this.merge({ cacheDirectory }, cacheOptions) : { cacheDirectory };
    return { loader: 'cache-loader', options };
  }
  ...
  createDllAssetPlugin() {
    const AssetFileWebpackPlugin = require('../config/plugin/AssetFileWebpackPlugin');
