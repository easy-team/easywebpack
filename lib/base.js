    ...
    Object.keys(plugins).forEach(name => {
      ...
      if (this.isUse(configInfo)) {
        ...
        if (utils.isObject(configInfo.name)) { // plugin object
          ...
          assert(Clazz, chalk.red(`dynamic create plugin[${name}] error, please check the npm module [${pluginName}] whether installed.
          if not installed, please execute the command [npm install ${pluginName} --save-dev] in command line`));
          if (configInfo.entry) {
