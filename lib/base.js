...
class WebpackBaseBuilder extends Config {
  ...
  mergeConfig(config = {}, needInitialize = true, sourceConfig) {
    ...
    if (needInitialize) {
      ...
      // manifest 兼容低版本
      Adapter.normalizeManifestPlugin(this.configPlugins, this.config);
    }
  }
      ...
      if (target[name]) {
        ...
        if (this.utils.isBoolean(configPlugin)) {
          ...
          if (configPlugin.args) {
            ...
            configPlugin.args = configPlugin; // 如果 name 和 args 不存在, configPlugin直接作为args, 配置是可以省去 args key
          }
      ...
      if (itemLoader.framework && this.styleLoader && this.styleLoader.loader) {
        if (extract) {
          ...
          const hasStyleLoader = itemLoader.use.find(loader => {
            return loader === this.styleLoader || (this.utils.isObject(loader) && loader.loader === this.styleLoader);
          });
          if (!hasStyleLoader) {
    ...
    dllArray.forEach(item => {
      if (item.name && item.lib) {
        ...
        plugin[`${item.name}Dll`] = new this.webpack.DllPlugin({
          ...
          path: WebpackBaseBuilder.getDllFilePath(item.name),
          name: '[name]'
        });
  ...
  createDllReferencePlugin(dll) {
    ...
    dllArray.forEach(item => {
      if (item.name && item.lib) {
        const plugin = {};
        const filename = WebpackBaseBuilder.getDllFilePath(item.name);
