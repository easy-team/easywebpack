...
class WebpackBaseBuilder {
  ...
  createWebpackPlugin() {
    ...
    Object.keys(this.configPlugin).forEach(name => {
      ...
      if (Utils.isTrue(configInfo.enable) || (Utils.isFunction(configInfo.enable) && configInfo.enable())) {
        if (Utils.isObject(configInfo.name)) { // plugin object initialize
          ...
          if (configInfo.require) {
            Clazz = Clazz[configInfo.require];
          }
          if (defaultConfig.args || customConfig.args) {
            ...
            const args = merge(defaultArgs, customArgs);
  ...
  initEnv(config = {}) {
    this.env = this.config.env || config.env; // local/dev, test, prod
    if (this.env === 'test') {
      this.dev = false;
      this.test = true;
      this.prod = false;
