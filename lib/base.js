...
class WebpackBaseBuilder {
  ...
  getDllFilePath(name) {
    return this.getCompileTempDir(`dll/manifest-${name}-dll.json`);
  }
      ...
      dllArray.forEach(item => {
        if (item.name && item.lib) {
          ...
          plugin[`${item.name}Dll`] = new this.webpack.DllPlugin({
            ...
            path: this.getDllFilePath(item.name),
            name: '[name]_[hash]'
          });
  ...
  createDllReferencePlugin(dll) {
    ...
    if (dll) {
      ...
      dllArray.forEach(item => {
        if (item.name && item.lib) {
          const plugin = {};
          const filename = this.getDllFilePath(item.name);
