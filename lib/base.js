...
class WebpackBaseBuilder {
  ...
  createPlugin(plugins) {
    ...
    Object.keys(plugins).forEach(name => {
      let configInfo = plugins[name];
      if (this.isUse(configInfo)) {
        let plugin;
        let pluginName;
        // 如果直接是 new Plugin 方式
        if (utils.isObject(configInfo) && configInfo.constructor && configInfo.constructor.prototype && configInfo.constructor.prototype.apply) {
          plugin = configInfo;
