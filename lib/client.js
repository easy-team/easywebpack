...
class WebpackClientBuilder extends WebpackBaseBuilder {
  ...
  initClientPlugin() {
    ...
    this.addPlugin(webpack.LoaderOptionsPlugin, () => Utils.isObject(this.miniCss) ? this.miniCss : { minimize: !!this.miniCss });
  }
  ...
  createHtmlPlugin() {
    ...
    Object.keys(this.htmlEntry).filter(entryName => !commonsChunk || !commonsChunk.some(chunk => chunk === entryName)).forEach(entryName => {
