  ...
  initHtml() {
    if (this.config.entry && (this.config.entry.template || this.config.entry.html || this.config.framework === 'html')) {
      ...
      if (this.isUse(plugin)) {
        ...
        const manifestDll = manifest && manifest.args && manifest.args.manifestDll;
        const dllCss = [];
        const dllScript = [];
        if (this.utils.isObject(manifestDll)) {
          Object.keys(manifestDll).forEach(key => {
            const url = manifestDll[key];
            if(this.utils.isString(url)){
              if(/\.js$/.test(url)){
                dllScript.push(url)
        ...
        Object.keys(entry).forEach(entryName => {
          if (!commonsChunk.includes(entryName)) {
            const entryTemplate = entry[entryName].replace(extMath, '.html');
            ...
            this.plugins[entryName] = this.merge(plugin, { args: { chunks, filename, template, dllCss, dllScript } });
          }
        });
        this.addEntry(entry);
      }
