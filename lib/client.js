  ...
  createHTML() {
    ...
    (this.config.template || this.config.entry.template || this.config.entry.html || this.config.framework === 'html')) {
      ...
      if (this.isUse(plugin)) {
        ...
        //   }
        // }
        const dllsChunk = this.getDllChunk();
        Object.keys(entry).forEach(entryName => {
          ...
          const commonsChunk = this.getCommonsChunk();
          const chunks = [].concat(dllsChunk).concat(commonsChunk).concat(entryName);
          if (!commonsChunk.includes(entryName)) {
            this.plugins[entryName] = this.merge({ args: { minify, inject, chunks, filename, template } }, plugin);
          }
        });
        this.addEntry(entry);
        if (dllsChunk.length > 0) {
          this.mergePlugin([new AssetFileWebpackPlugin({
            env: this.env,
            chunks: dllsChunk,
