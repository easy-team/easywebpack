...
class WebpackClientBuilder extends WebpackBaseBuilder {
  constructor(config) {
    ...
    this.setHot(this.dev);
    this.setExtractCss(!this.dev);
    this.initEasyWebpackClient();
  }

  initEasyWebpackClient() {
  ...
  addHotEntry() {
    if (this.config.hot) {
      ...
      hotMiddleware.pop();
      const hotConfig = `${path.posix.join(hotMiddleware.join(path.sep))}/client?path=http://${Utils.getIp()}:${this.config.port + 1}/__webpack_hmr&noInfo=false&reload=false&quiet=false`;
  ...
  addHtml() {
    ...
    if (html && html.include) {
      ...
      Object.keys(entry).forEach(entryName => {
        ...
        const htmlDir = html.buildDir || this.confprefix;
