...
exports.getBuilderConfig = (config = {}, option = {}) => {
  ...
  if (utils.isString(config)) {
    ...
    if (fs.existsSync(filepath)) {
      builderConfig = require(filepath);
    } else {
      builderConfig = {};
    }
  } else if (utils.isObject(config) && (config.cli || config.entry || config.framework || config.egg || config.lib || config.dll)) {
    builderConfig = config;
  } else if (fs.existsSync(defaultWebpackFilepath)) {
    builderConfig = merge(require(defaultWebpackFilepath), config);
  } else {
    builderConfig = config;
  }
  if (!builderConfig.baseDir) {
    builderConfig.baseDir = baseDir;
  }
  if (!builderConfig.framework && option.framework) {
    builderConfig.framework = option.framework;
  }
