...
exports.getBuilderConfig = (config = {}, option = {}) => {
  ...
  if (utils.isObject(config)) {
    ...
    if (fs.existsSync(pkgFile)) {
      ...
      return merge(baseConfig, pkg.webpack, config);
    }
...
exports.getConfig = (config, option) => {
  // cli 已经初始化配置，直接返回
  if (utils.isObject(config) && config.cli) {
    return config;
  }
  return exports.getBuilderConfig(config, option);
};

exports.getWebpackConfig = (config, builders, option = {}) => {
  const easyConfig = exports.getConfig(config, option);
