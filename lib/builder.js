...
exports.getConfig = (config, option) => {
  ...
  if (!easyConfig.framework) {
    const framework = exports.getFramework(easyConfig.baseDir);
    if (!framework) {
      throw Error('webpack config no set framework!');
    }
    easyConfig.framework = framework;
  }
  // 自动检测动态设置 webpack target, 默认返回 target:web 和 target:node 两个 webpack config
  if (easyConfig.target !== null && (easyConfig.template || !utils.isEgg(config))) {
