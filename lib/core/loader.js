    ...
    Object.keys(loaders).forEach(name => {
      ...
      if (Array.isArray(itemLoader.use)) {
        ...
        if (itemLoader.framework && this.styleLoader && this.styleLoader.loader) {
          if (extract) {
            const niniCssPluginName = 'mini-css-extract-plugin';
            const niniCssPluginFile = path.join(this.baseDir, 'node_modules', niniCssPluginName);
            const MiniCssExtractPlugin = fs.existsSync(niniCssPluginFile) ? require(niniCssPluginFile) : require(niniCssPluginName);
