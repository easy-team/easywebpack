...
class WebpackBaseBuilder {
  ...
  initConfig() {
    ...
    this.buildPath = Utils.normalizePath(this.config.build.path, this.config.baseDir);
  ...
  initOption() {
    const entryDirs = Array.isArray(this.config.build.entry) ? this.config.build.entry : [this.config.build.entry];
    const normalizeEntryDirs = entryDirs.map(entryDir => Utils.normalizePath(entryDir, this.config.baseDir));
    this.options = {
      entry: Utils.getEntry(normalizeEntryDirs),
      resolve: {
        extensions: ['.js']
      },
      plugins: [
  ...
  initPlugin() {
    ...
    this.addPlugin(webpack.optimize.UglifyJsPlugin, () => {
      ...
      return Utils.isObject(this.miniJs) ? merge(defaultOption, this.miniJs) : defaultOption;
    }, () => this.miniJs);
  ...
  addDefinePlugin(defines) {
    this.addPlugin(webpack.DefinePlugin, defines);
  }
  ...
  setCDN(cdnUrl, cdnDynamicDir) {
    const cdnDir = cdnDynamicDir || this.config.build.cdnDynamicDir;
    const publicPath = cdnDir ? Utils.joinPath(cdnUrl, cdnDir) : cdnUrl;
  ...
  setDevMode() {
    const host = Utils.getHost(this.config);
  ...
  create() {
    this.setOption({
      output: {
        path: this.buildPath,
        publicPath: `${this.publicPath.replace(/\/$/, '')}/`
      }
    });
  ...
  setMiniCss(miniCss) {
    this.miniCss = Utils.isBoolean(miniCss) ? miniCss : merge(this.miniCss, miniCss);
  }

  setMiniImage(miniImage) {
    this.miniImage = Utils.isBoolean(miniImage) ? miniImage : merge(this.miniImage, miniImage);
  }

  setMiniJs(uglify) {
    this.setUglifyJs(uglify);
  }

  setUglifyJs(uglify) {
    this.miniJs = Utils.isBoolean(uglify) ? uglify : merge(this.miniJs, uglify);
  }
